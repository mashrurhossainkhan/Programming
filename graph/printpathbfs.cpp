...
void bfs(int s,int vertices,int arr[100][100]){
	...
	cout<<"print-path:"<<endl<<"Enter the source and destination:";
	int pathsource,pathdestination;
	
	cin>>pathsource>>pathdestination;
	
	for(int i=pathsource; i<=pathdestination; i++){
		if(pathsource==pathdestination){
			cout<<pathsource<<endl;
		}else if(prev[i]==123456789){
			cout<<"No path"<<endl;
			break;
		}else{
			cout<<i<<endl;
