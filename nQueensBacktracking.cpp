...
int main(){
	...
	cout<<"---------------10 represents the queen---------------\n";
	cout<<"---------------15 represents the empty cells ---------------\n";
	cout<<"---------------9 represents there was a queen ---------------\n";
	cout<<"---------------other numbers represent that the cells are blocked---------------\n";
			...
			for(i=n-1; i>=0; i--){
				...
				for(j=n-1; j>=0; j--){
					if(board[i][j]==10){
						...
						count=Closeblocked( i, j, n, board, count);
						draw(board,n);
						flag=1;
						k++;
						break;
